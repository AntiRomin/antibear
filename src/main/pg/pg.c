...
bool pgLoad(const pgRegistry_t* reg, const void *from, uint32_t size, uint8_t version)
{
    pgResetInstance(reg, pgOffset(reg));
    // restore only matching version, keep defaults otherwise
    if (version == pgVersion(reg)) {
        const uint32_t take = MIN(size, pgSize(reg));
...
uint32_t pgStore(const pgRegistry_t* reg, void *to, uint32_t size)
{
    const uint32_t take = MIN(size, pgSize(reg));
